...
from database import init_db, create_user, get_user_by_email, get_user_by_id, save_strategy, get_strategies, delete_strategy, check_password
...
def _run_strategy_backtest(strategy, ohlcv, settings):
...
def register():
    ...
    if not user_id:
        return jsonify({'error': True, 'message': 'Email already exists'}), 409
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=min(len(strategies), 5)) as executor:
            results = list(executor.map(
                lambda strategy: _run_strategy_backtest(strategy, ohlcv, settings),
                strategies
            ))
...
def robustness_test():
    ...
    try:
        ...
        cache_key = build_robustness_cache_key(code, strategy_name, years, intensities, initial_capital)
        _cleanup_old_robustness_cache()
        cached = robustness_cache.get(cache_key)
        if cached:
            return jsonify(cached["payload"]), 200

        result = run_robustness_test(code, strategy_name, years, intensities, initial_capital)
